...
export function setModelInputData(renderState: IRenderState, gptModel: IGpuGptModel) {
    let { gl } = renderState;
    let { inputTokens, shape: { B, T } } = gptModel;

    // The pinned upstream initial sequence: C B A B B C, then unused positions.
