...
// Visual port of Walkthrough04_SelfAttention.tsx (no React, commentary, or sidebar).
...
const Black = new Vec4(0, 0, 0);

export function runDeveloSelfAttention(args: IDeveloWalkthroughArgs) {
    ...
    if (t_focusQCol.t > 0) {
        ...
        if (t_moveDotCells.t > 0 && t_moveToDest.t === 0.0) {
            ...
            for (let c = 0; c < C; c++) {
                ...
                if (cellTimesSymT > 0.0 && t_dotCellsZoomOut.t == 0.0) {
                    ...
                    if (c > 0 && cellTimesSymT > 0.0) {
                        ...
                        let fontOpts: IFontOpts = { color: Black, size: 1.5, mtx };
                ...
                if (cellMoveT >= 1.0) {
                    drawSymbolBetweenBlocks(args, qCells[c], inCells[c], Dim.X, "x", { size: 1.5, color: Black });
                }
            }
            ...
            if (t_addBias.t >= 0.0) {
                ...
                if (t_addBias.t > 0.4) {
                    drawSymbolBetweenBlocks(args, qBiasCell, qCells[qCells.length - 1], Dim.X, "+", { size: 1.5, color: Black });
                }
            }
        }
        ...
        if (t_expandVCols.t > 0 && t_placeVOutput.t <= 0) {
            ...
            for (let i = 0; i < attnExampleIdx + 1; i++) {
                ...
                if (t_moveAttnVals.t > 0.8 && t_applyMultiplies.t < 0.7) {
                    drawSymbolBetweenBlocks(args, allVCols[i], allAttnCells[i], Dim.X, 'x', { color: Black, size: 1.5 });
                }
                ...
                if (t_applyMultiplies.t > 0.6 && i > 0 && t_applyAdds.t < 0.7) {
                    drawSymbolBetweenBlocks(args, allVCols[i - 1], allVCols[i], Dim.X, '+', { color: Black, size: 1.5 });
                }
            }
        }
...
export function focusSelfAttentionHeadTimers(args: IDeveloWalkthroughArgs, duration: number) {
...
export function focusSelfAttentionHead(args: IDeveloWalkthroughArgs, timers: ReturnType<typeof focusSelfAttentionHeadTimers>) {
    let { layout } = args;
...
export function drawSymbolBetweenBlocks(args: IDeveloWalkthroughArgs, block1: IBlkDef, block2: IBlkDef, dim: Dim, symbol: string, opts: { color: Vec4, size: number }) {
    let { color, size } = opts;
